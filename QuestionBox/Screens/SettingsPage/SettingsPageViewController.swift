...
extension SettingsPageViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .notPremiumAccount:
            if let NotPremiumSettingsMenuItems = viewModel?.menuItems {
                cell?.setUpNotPremiumAccount(notPremiumSettingsMenuItems: NotPremiumSettingsMenuItems[indexPath.row])
            }
        case .premiumAccount:
            if let premiumAccount = viewModel?.premiumSettingsPageModel {
                cell?.setUpPremiumAccount(premiumSettingsPageModel: premiumAccount[indexPath.row])
            }
        case .none:
            break
        }
